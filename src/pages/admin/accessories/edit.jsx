...
const AdminEditAccessoryPage = () => {
  ...
  const [category, setCategory] = useState('Pulseira')
  ...
  useEffect(() => {
    ...
    fetch(`${base}/accessories/${id}`)
      .then(res => res.json())
      .then(data => {
        setTitle(data.title || data.name || '')
        setPrice(String(data.price ?? ''))
        setCategory(data.category || 'Pulseira')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${base}/accessories/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      })
      if (!res.ok) throw new Error('Falha ao salvar o acessório')
        ...
        {!loading && (
          <form onSubmit={handleSubmit} className="add-product-form">
            ...
            <div className="form-group">
              ...
              <input id="stockStatus" type="text" value={stockStatus} readOnly placeholder="Será preenchido automaticamente" />
            </div>
...
export default AdminEditAccessoryPage